...
class TestExitCleanlyOnConfigError:
    ...
    def test_non_config_error_propagates(self) -> None:
        ...
        # Non-ConfigError exceptions must NOT be swallowed (no clean exit).
    ...
    def test_config_error_base_catches_subclasses(self) -> None:
        # NoServerInstancesError is a ConfigError, so the decorator's `except ConfigError` catches it.
